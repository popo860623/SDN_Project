...
class CustomTopo(Topo):
    ...
    def __init__(self,**opts):
        ...
        for i in range(client_per_node):
            ...
            self.addLink(self.s[2],self.client[i+client_per_node],bw=10,delay='100ms')
            self.addLink(self.s[4],self.client[i+2*client_per_node],bw=10,delay='100ms')

        self.addLink(self.s[1],self.server[0],bw=10,delay='100ms')
        self.addLink(self.s[3],self.server[1],bw=10,delay='100ms')	
...
def createTopo():
    ...
    print net.hosts[-3].cmd('python ../server.py 5001 '+sys.argv[1]+'/Server'+str(1)+'.log &')
    print net.hosts[-2].cmd('python ../server.py 5001 '+sys.argv[1]+'/Server'+str(2)+'.log &')
    print net.hosts[-1].cmd('python ../server.py 5001 '+sys.argv[1]+'/Server'+str(3)+'.log &')
    ...
    for i in a :
        print net.hosts[i].cmd('python ../client.py '+sys.argv[1]+'/client'+str(i)+'.csv '+str(net.hosts[-3].IP())+' 5001 &')
        time.sleep(3)
