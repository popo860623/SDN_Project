...
def addrouting(servicePort, MainserverIP, NewserverIP, clientIP):
    ...
    matchTo = parser.OFPMatch(eth_src=mac_client, eth_dst=mac_Oldserver, eth_type=0x0800,
                              ip_proto=17, ipv4_src=clientIP, ipv4_dst=MainserverIP, udp_dst=servicePort)
    ...
    instTo = [parser.OFPInstructionActions(
        ofproto.OFPIT_APPLY_ACTIONS, actionsTo)]
    modTo = parser.OFPFlowMod(
        datapath=datapath, priority=4097, match=matchTo, instructions=instTo)
    datapath.send_msg(modTo)

    matchBack = parser.OFPMatch(eth_src=mac_Newserver, eth_dst=mac_client, eth_type=0x0800,
                                ip_proto=17, ipv4_src=NewserverIP, ipv4_dst=clientIP, udp_src=servicePort)
    ...
    instBack = [parser.OFPInstructionActions(
        ofproto.OFPIT_APPLY_ACTIONS, actionsBack)]
    modBack = parser.OFPFlowMod(
        datapath=datapath, priority=4097, match=matchBack, instructions=instBack)
...
def delrouting(servicePort, MainserverIP, NewserverIP, clientIP):
    ...
    modTo = parser.OFPFlowMod(datapath=datapath, priority=4097, match=matchTo, instructions=instTo,
                              command=datapath.ofproto.OFPFC_DELETE, out_port=ofproto.OFPP_ANY, out_group=ofproto.OFPG_ANY)
    datapath.send_msg(modTo)
