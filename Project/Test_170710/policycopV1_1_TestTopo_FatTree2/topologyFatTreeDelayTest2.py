...
# sys.argv[1] = path to save result's folder
    ...
    for i in range(0,2*Fat_Tree_Level) :
        print net.hosts[(-1)*(i+1)].cmd('python ../server.py 5001 '+sys.argv[1]+'/Server'+str(2*Fat_Tree_Level-i)+'.log &')
    ...
    for i in a :
        print net.hosts[i].cmd('python ../client.py '+sys.argv[1]+'/client'+str(i)+'.csv '+str(net.hosts[(-2)*Fat_Tree_Level].IP())+' 5001 &')
        time.sleep(3)
