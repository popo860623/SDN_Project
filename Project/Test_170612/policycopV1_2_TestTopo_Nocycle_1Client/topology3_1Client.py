...
class CustomTopo(Topo):
    ...
    def __init__(self,**opts):
        ...
        self.addLink(self.s[1],self.s[3],bw=10,delay='3ms')
        self.addLink(self.s[1],self.s[4],bw=10,delay='3ms')
        ...
        for i in range(client_per_node):
            self.addLink(self.s[3],self.client[i])
            self.addLink(self.s[4],self.client[i+client_per_node])
            self.addLink(self.s[5],self.client[i+2*client_per_node])

        self.addLink(self.s[0],self.server[0])
        self.addLink(self.s[3],self.server[1])	
        self.addLink(self.s[4],self.server[2])
...
def createTopo():
    ...
    print a
    for i in a :
        print net.hosts[i].cmd('iperf -c '+str(net.hosts[-3].IP())+' -p 5001 -i 1 -t 300 -u -w 2048K &')
