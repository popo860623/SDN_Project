...
class CustomTopo(Topo):
    ...
    def __init__(self,**opts):
        ...
        for i in range(client_per_node):
            self.addLink(self.s[0],self.client[i],delay='100ms')
            self.addLink(self.s[2],self.client[i+client_per_node],delay='100ms')
            self.addLink(self.s[4],self.client[i+2*client_per_node],delay='100ms')

        self.addLink(self.s[1],self.server[0],delay='100ms')
        self.addLink(self.s[3],self.server[1],delay='100ms')	
        self.addLink(self.s[5],self.server[2],delay='100ms')
